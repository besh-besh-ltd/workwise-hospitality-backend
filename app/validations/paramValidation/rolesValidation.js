...
const schema_posts = {
    ...
    try {
      ...
      upload(req, res, async function (err) {
        if (err) {
          ...
          const result = schemas.create_subadmin.validate(req.body, {
            abortEarly: false
          });
          if (result.error) {
            let err_msg = {};
  ...
  update_subadmin: async (req, res, next) => {
    try {
      let upload = multer({
        storage: store_profile_images,
        limits: {
      ...
      upload(req, res, async function (err) {
        if (err) {
          ...
          const result = schemas.update_subadmins.validate(req.body, {
            abortEarly: false
          });
          if (result.error) {
            let err_msg = {};
