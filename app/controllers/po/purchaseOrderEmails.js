...
export const sendApprovalNotification = async (purchaseOrder, userId) => {
  return new Promise(async (resolve, reject) => {
    ...
    let product = await db.oneOrNone(
      `SELECT P.id, P.name FROM tbl_rfq_items trp JOIN tbl_product_variant P ON P.id = trp.product_variant_id WHERE trp.id = $1`,
...
export const sendPONotificationToVendor = async (purchaseOrder, user) => {
    try {
        ...
        const product = await db.one(
            `SELECT P.id, P.name FROM tbl_rfq_items TRP JOIN tbl_product_variant P ON TRP.product_variant_id = P.id WHERE TRP.id = $1`,
            [purchaseOrder.rfq_product_id]
        )
