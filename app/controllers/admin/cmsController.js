...
const cmsController = {
  ...
  productTechSpecAdd : async (req , res , next) => {
    try {

      const { productId, techSpec } = req.body;

      const response = await cmsModel.saveProductTechSpec(productId, techSpec);
      
      if (response) {
        res.status(200).json({
          status: 1,
          message: 'Product Tech Spec added successfully'
        }).end();

      } else {
        res.status(400).json({
          status: 3,
          message: Config?.errorText?.value || "Failed to add product tech spec"
        }).end();
      }
