 ...
 *
 * Business-unit clean slate (runs ONCE before any sheet rows are processed):
 *   - ALL tbl_user_role_scopes for (company_id, hotel_id) are deleted — every user
 *     scoped to this hotel loses every role, not just users present in the sheet.
 *   - ALL tbl_hospitality_user_mappings (mapping_type=1) for this hotel are deleted —
 *     every user's access to this business unit is removed.
 *   - The DB is always fully cleared, but the report shows only NET removals:
 *     a scope/access the new sheet re-adds identically is treated as "kept"
 *     (shown only as an insert); one the sheet does NOT re-add is shown as a
 *     removal (WOULD_DELETE / DELETED) in both the Changes and Removals sheets.
 *   - Sheet users get their roles/access re-added during the apply pass;
 *     users absent from the sheet stay net-removed from this unit.
 *
 * Wipe & replace strategy (per-user, after the clean slate):
...
const FILE_PATH = fileArg
  ...
  : path.resolve(process.env.HOME || '', 'Downloads/sheet_khil_updated.xlsx');
...
const removals = [];     // business-unit pre-wipe log (roles/access removed) for Removals sheet
...
function logRemoval(entry) { removals.push(entry); }
function bumpSummary(userId, userName, key) {
  if (!userSummary[userId]) {
    userSummary[userId] = { user_id: userId, user_name: userName, role_scopes: 0, depts: 0, mapping_created: 0, removed: 0, errors: 0 };
  }
...
// Identity of a role-scope row for net diffing (company_id/hotel_id are constant
// across the wipe set and the re-inserts, so the key is user + role + department).
function scopeKey(userId, roleId, departmentId) {
  return `${userId}|${roleId}|${departmentId == null ? 'NULL' : departmentId}`;
}
...
// ─── Business-unit pre-wipe (clean slate) ───────────────────────────────────

/**
 * Before re-populating from the sheet, completely clear the business unit:
 *   - Remove every role scope of EVERY user scoped to this hotel
 *       (tbl_user_role_scopes WHERE company_id = X AND hotel_id = Y)
 *   - Remove every user's hotel access to this hotel
 *       (tbl_hospitality_user_mappings WHERE company/hotel match AND mapping_type = 1)
 *
 * The DB is always wiped to a clean slate. For REPORTING, however, only the
 * NET removals are logged: a pre-existing scope/access that the new sheet will
 * re-insert identically is treated as "kept" (it shows up only as an insert in
 * the apply pass), while a scope/access the sheet does NOT re-add is logged as a
 * net removal (WOULD_DELETE / DELETED) in both the Changes and Removals sheets.
 *
 *   afterScopeKeys    — Set of scopeKey() for every scope the sheet will insert
 *   afterMappingUsers — Set of user_id that will keep hotel access (in the sheet)
 *
 * In dry-run mode nothing is deleted — net removals are logged as WOULD_DELETE.
 */
async function wipeBusinessUnit(companyId, hotelId, deptMap, afterScopeKeys, afterMappingUsers) {
  // 1. Snapshot role scopes about to be removed (any user in this hotel)
  const scopeRows = await db.any(
    `SELECT urs.user_id,
            u.name  AS user_name,
            u.email AS email,
            urs.role_id,
     ...
     LEFT JOIN tbl_users u ON u.id = urs.user_id
     LEFT JOIN tbl_roles r ON r.id = urs.role_id
     WHERE urs.company_id = $1 AND urs.hotel_id = $2
     ORDER BY urs.user_id, urs.role_id`,
    [companyId, hotelId]
  );

  // 2. Snapshot hotel-access mappings about to be removed (any user in this hotel)
  const mappingRows = await db.any(
    `SELECT hum.user_id,
            u.name  AS user_name,
            u.email AS email
     FROM tbl_hospitality_user_mappings hum
     LEFT JOIN tbl_users u ON u.id = hum.user_id
     WHERE hum.hospitality_company_id = $1
       AND hum.hospitality_hotel_id = $2
       AND hum.mapping_type = 1
     ORDER BY hum.user_id`,
    [companyId, hotelId]
  );

  // 3. Delete (live mode only) — atomic
  let scopesDeleted = scopeRows.length;
  let mappingsDeleted = mappingRows.length;
  if (!DRY_RUN) {
    await db.tx(async t => {
      const rScopes = await t.result(
        `DELETE FROM tbl_user_role_scopes WHERE company_id = $1 AND hotel_id = $2`,
        [companyId, hotelId]
      );
      const rMaps = await t.result(
      ...
      scopesDeleted = rScopes.rowCount;
      mappingsDeleted = rMaps.rowCount;
    });
  }

  const removeAction = DRY_RUN ? 'WOULD_DELETE' : 'DELETED';

  // Keep only NET removals: scopes/access the new sheet will NOT re-add.
  // Anything the sheet re-inserts identically is "kept" and shows only as an
  // insert in the apply pass — not as a removal.
  const netScopeRemovals = scopeRows.filter(
    s => !afterScopeKeys.has(scopeKey(s.user_id, s.role_id, s.department_id))
  );
  const netMappingRemovals = mappingRows.filter(
    m => !afterMappingUsers.has(m.user_id)
  );

  // 4. Log each NET removed role scope — to both the Removals sheet AND the
  //    main Changes log (so net deletions are visible alongside WOULD_INSERT,
  //    including for users the new sheet does not re-add at all).
  for (const s of netScopeRemovals) {
    const deptName = s.department_id == null
    ...
    logRemoval({
      ...
      action: removeAction,
      role_id: s.role_id,
      role_title: s.role_title || `#${s.role_id}`,
    ...
    logChange({
      ...
      user_id: s.user_id,
      user_name: s.user_name || `#${s.user_id}`,
      rule: 'Business-Unit Clean Slate',
      action: removeAction,
      role_id: s.role_id,
      role_title: s.role_title || `#${s.role_id}`,
      department_id: s.department_id ?? '',
      ...
      hotel_id: hotelId,
      notes: 'Net removal — role scope not re-added by the new sheet',
    });
    bumpSummary(s.user_id, s.user_name || `#${s.user_id}`, 'removed');
  }

  // 5. Log each NET removed hotel-access mapping — Removals sheet + Changes log
  for (const m of netMappingRemovals) {
    logRemoval({
      user_id: m.user_id,
      user_name: m.user_name || `#${m.user_id}`,
      email: m.email || '',
      type: 'HOTEL_ACCESS',
      action: removeAction,
      role_id: '',
    ...
    logChange({
      ...
      company_id: companyId,
      hotel_id: hotelId,
      notes: 'Net removal — hotel access not re-added by the new sheet',
    });
    bumpSummary(m.user_id, m.user_name || `#${m.user_id}`, 'removed');
  }

  const distinctUsers = new Set([
    ...netScopeRemovals.map(s => s.user_id),
    ...netMappingRemovals.map(m => m.user_id),
  ]);

  return {
    scopesDeleted,                       // total physically deleted (clean slate)
    mappingsDeleted,                     // total physically deleted (clean slate)
    netScopesRemoved: netScopeRemovals.length,
    netMappingsRemoved: netMappingRemovals.length,
...
async function main() {
  ...
  // 7. PLAN pass — resolve users + build scopes (no DB writes). Collects the
  //    "after" state so the wipe can log only NET removals.
  console.log('\n--- Planning ---');
  const plan = [];                       // [{ excelRowNum, name, user, scopes, userDeptId }]
  const afterScopeKeys = new Set();      // scopeKey() for every scope to be inserted
  const afterMappingUsers = new Set();   // user_ids that will keep hotel access
  ...
  for (const { row, excelRowNum } of activeRows) {
    ...
    if (!user) {
      ...
      plan.push({ excelRowNum, name, user: null });
      continue;
    }
    ...
    plan.push({ excelRowNum, name, user, scopes, userDeptId });
    for (const s of scopes) afterScopeKeys.add(scopeKey(user.id, s.role_id, s.department_id));
    afterMappingUsers.add(user.id);
  }

  // 8. Business-unit clean slate — wipe ALL existing roles & hotel access for
  //    this unit, then log only the NET removals (those the sheet won't re-add).
  const preCounts = await db.one(
    `SELECT
       (SELECT COUNT(*) FROM tbl_user_role_scopes
          WHERE company_id = $1 AND hotel_id = $2) AS scopes,
       (SELECT COUNT(DISTINCT user_id) FROM tbl_user_role_scopes
          WHERE company_id = $1 AND hotel_id = $2) AS scope_users,
       (SELECT COUNT(*) FROM tbl_hospitality_user_mappings
          WHERE hospitality_company_id = $1 AND hospitality_hotel_id = $2 AND mapping_type = 1) AS mappings`,
    [companyId, hotelId]
  );
  console.log(
    `\nBusiness-unit clean slate: ${preCounts.scopes} role scope(s) across ` +
    `${preCounts.scope_users} user(s) and ${preCounts.mappings} hotel-access mapping(s) ` +
    `currently exist for hotel #${hotelId}.`
  );
  if (!DRY_RUN && (Number(preCounts.scopes) > 0 || Number(preCounts.mappings) > 0)) {
    const confirm = await ask('Proceed with clearing existing roles/access for this unit? (yes/no): ');
    if (confirm.trim().toLowerCase() !== 'yes') {
      console.error('✗ Aborted by user — no changes made.');
      pgp.end();
      process.exit(1);
    }
  }
  const wipeResult = await wipeBusinessUnit(companyId, hotelId, deptMap, afterScopeKeys, afterMappingUsers);
  console.log(
    `Cleared ${wipeResult.scopesDeleted} role scope(s) + ${wipeResult.mappingsDeleted} mapping(s). ` +
    `Net ${DRY_RUN ? 'would be removed' : 'removed'} (not re-added by sheet): ` +
    `${wipeResult.netScopesRemoved} role scope(s), ${wipeResult.netMappingsRemoved} hotel-access ` +
    `mapping(s), affecting ${wipeResult.distinctUsers} user(s).\n`
  );

  // 9. APPLY pass — write the planned scopes/dept/mapping for each resolved user.
  console.log('--- Processing users ---');
  let okCount = 0;
  let failCount = 0;

  for (const { excelRowNum, name, user, scopes, userDeptId } of plan) {
    if (!user) { failCount++; continue; }  // unresolved — already logged in plan pass
...
async function writeReport(hotel) {
  ...
  // Sheet 1b: Removals (business-unit clean slate)
  const wsR = wb.addWorksheet('Removals');
  wsR.columns = [
    { header: 'User ID', key: 'user_id', width: 10 },
    { header: 'User Name', key: 'user_name', width: 24 },
    { header: 'Email', key: 'email', width: 32 },
    { header: 'Removed', key: 'type', width: 16 },
  ...
  styleHeader(wsR);
  for (const row of removals) wsR.addRow(row);
  ...
  ws3.columns = [
    ...
    { header: 'Net Removed', key: 'removed', width: 14 },
  ...
  ws4.addRows([
    ...
    { field: 'Role Scopes Net Removed', value: removals.filter(r => r.type === 'ROLE_SCOPE').length },
    { field: 'Hotel Access Net Removed', value: removals.filter(r => r.type === 'HOTEL_ACCESS').length },
    { field: 'Users Net Removed', value: new Set(removals.map(r => r.user_id)).size },
  ]);
